...
# 设置参数
p = 4  # 非线性项的幂次
lambda_values = np.logspace(0, 4, 6)  # λ 的值从 10^0 到 10^4，取6个点
tol = 1e-6  # 收敛容差
...
# 绘制 u(x) 随 λ 变化的曲线
fig, ax = plt.subplots(len(lambda_values), 1, figsize=(10, 12))  # 增加行数来显示6个图表

# 对每个 λ 的解绘制曲线
for i, lambda_val in enumerate(lambda_values):
    ax[i].plot(range(len(grid_graph.nodes)), solutions[i], marker='o', label=f'λ = {lambda_val:.1e}')
    ax[i].set_title(f'Solution for λ = {lambda_val:.1e}')
    ax[i].set_xlabel('Node index')
    ax[i].set_ylabel('u(x)')
    ax[i].legend()
    ax[i].grid(True)  # 添加网格
